"""
SQL 查询技能
用于处理用户关于 SQL 查询的问题，从 Milvus 中获取表格 schema 信息并生成正确的 SQL 语句
"""
import ollama
...
SKILL_DESCRIPTION = "SQL 查询技能，根据用户问题和表结构信息生成正确的 SQL 语句"
...
def sql_query_skill(query: str) -> str:
    """
    SQL 查询技能入口函数
...
SKILL_FUNCTIONS = {
    "sql_query": {
        "function": sql_query_skill,
        "description": "根据用户问题和表结构信息生成正确的 SQL 语句",
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "用户的 SQL 查询问题"
                }
            },
            "required": ["query"]
        }
...
if __name__ == "__main__":
    ...
    result = sql_query_skill("统计越南用户的订单总额需要哪些表？")
    print(result)